...
from ..util import patch, write_script


class FirefoxRecipe(GnuRecipe):
    def __init__(self, *args, **kwargs):
        super(FirefoxRecipe, self).__init__(*args, **kwargs)
        self.sha256 = 'c2cae016089e816c03283a359c582efa' \
                      'b3bca34e6048ecc2382b43c1eb342457'
        self.name = 'firefox'
        self.version = '57.0'
        self.url = 'https://ftp.mozilla.org/pub/firefox/releases/' \
                   '$version/linux-x86_64/en-US/' \
                   'firefox-$version.tar.bz2'

        self.description = r'''
Requirements from https://www.mozilla.org/en-US/firefox/57.0/system-requirements/
GNU/Linux
...
And yet:
$ /usr/bin/ldd firefox
	linux-vdso.so.1 (0x00007ffd730e7000)
	libpthread.so.0 => /lib64/libpthread.so.0 (0x00007f9551196000)
	libdl.so.2 => /lib64/libdl.so.2 (0x00007f9550f92000)
	librt.so.1 => /lib64/librt.so.1 (0x00007f9550d8a000)
	libstdc++.so.6 => /lib64/libstdc++.so.6 (0x00007f9550a02000)
	libm.so.6 => /lib64/libm.so.6 (0x00007f95506ec000)
	libgcc_s.so.1 => /lib64/libgcc_s.so.1 (0x00007f95504d5000)
	libc.so.6 => /lib64/libc.so.6 (0x00007f9550104000)
	/lib64/ld-linux-x86-64.so.2 (0x00007f95513b5000)
'''
    ...
    def install(self):
        ...
        dst = os.path.join(self.prefix_dir, 'firefox')
...
%s/firefox "$@"
''' % dst
        filename = os.path.join(self.prefix_dir, 'bin', 'firefox')
