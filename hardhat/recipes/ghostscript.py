...
class GhostScriptRecipe(GnuRecipe):
    def __init__(self, *args, **kwargs):
        ...
        self.fonts = Extra()
        self.fonts.url = 'http://downloads.sourceforge.net/gs-fonts/' \
                         '$name-$version.tar.gz'
        ...
        self.fonts2 = Extra()
        self.fonts2.url = 'http://downloads.sourceforge.net/gs-fonts/' \
                          '$name-$version.tar.gz'
        self.fonts2.version = '6.0'
        self.fonts2.name = 'gnu-gs-fonts-other'
        self.fonts2.sha256 = '8146cccc4699fe9dab841446bdd17039' \
                             'f4769c903eceb54409188b920754aab3'

        self.extra_downloads.append(self.fonts2)
    ...
    def install(self):
        ...
        args = ['tar', '-xvf', self.fonts2.filename, '-C',
                '%s/share/ghostscript' % self.prefix_dir, '--no-same-owner']
        self.run_exe(args, self.directory, self.environment)

        args = ['fc-cache', '-v',
                '%s/share/ghostscript/fonts/' % self.prefix_dir]
        self.run_exe(args, self.directory, self.environment)
